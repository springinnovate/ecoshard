...
import ecoshard.geoprocessing.symbolic
from ecoshard.geoprocessing.geoprocessing_core import \
    DEFAULT_GTIFF_CREATION_TUPLE_OPTIONS
from ecoshard.geoprocessing.geoprocessing import _assert_is_valid_pixel_size
...
def _geometry_to_vector(
    ...
    """Passthrough to geoprocessing.shapely_geometry_to_vector."""
    projection = osr.SpatialReference()
    projection.ImportFromEPSG(projection_epsg)
    geoprocessing.shapely_geometry_to_vector(
...
def _array_to_raster(
    ...
    """Passthrough to geoprocessing.array_to_raster."""
    projection = osr.SpatialReference()
    projection_wkt = None
...
class TestGeoprocessing(unittest.TestCase):
    """Tests for geoprocessing.geoprocessing."""
    ...
    def test_star_import(self):
        ...
        # Actually trying out the `from ecoshard.geoprocessing import *` notation here
        # raises a SyntaxWarning.  Instead, I'll just ensure that every
        # attribute in geoprocessing.__all__ is a function that is available
        # at the ecoshard.geoprocessing level.
        import inspect
        for attrname in geoprocessing.__all__:
            try:
                func = getattr(ecoshard.geoprocessing, attrname)
                self.assertTrue(
                    isinstance(func, (
                ...
                self.fail(('Function %s is in geoprocessing.__all__ but '
    ...
    def test_reclassify_raster_no_raster_path_band(self):
        """geoprocessing: test reclassify raster is path band aware."""
        n_pixels = 9
        pixel_matrix = numpy.ones((n_pixels, n_pixels), numpy.float32)
        test_value = 0.5
        pixel_matrix[:] = test_value
    ...
    def test_reproject_vector_partial_fields(self):
        """geoprocessing: reproject vector with partial field copy."""
        # Create polygon shapefile to reproject
        base_srs = osr.SpatialReference()
        # NAD83(CSRS) / UTM zone 10N
        base_srs.ImportFromEPSG(3157)
    ...
    def test_reproject_vector_latlon_to_utm(self):
        """geoprocessing: reproject vector from lat/lon to utm."""
        # Create polygon shapefile to reproject
        base_srs = osr.SpatialReference()
        # WGS84
    ...
    def test_reproject_vector_utm_to_latlon(self):
        """geoprocessing: reproject vector from utm to lat/lon."""
        # Create polygon shapefile to reproject
        base_srs = osr.SpatialReference()
        # NAD83(CSRS) / UTM zone 10N
        base_srs.ImportFromEPSG(3157)
    ...
    def test_reproject_vector_utm_to_utm(self):
        """geoprocessing: reproject vector from utm to utm."""
        # Create polygon shapefile to reproject
        base_srs = osr.SpatialReference()
        # NAD83(CSRS) / UTM zone 10N
        base_srs.ImportFromEPSG(3157)
    ...
    def test_calculate_disjoint_polygon_set(self):
        """geoprocessing: test calc_disjoint_poly no/intersection."""
        ...
        result = geoprocessing.calculate_disjoint_polygon_set(
            vector_path, bounding_box=[-10, -10, -9, -9])
        self.assertTrue(not result)

        # otherwise none overlap:
        result = geoprocessing.calculate_disjoint_polygon_set(vector_path)
        self.assertEqual(len(result), 1, result)

    def test_zonal_stats_for_small_polygons(self):
        """geoprocessing: test small polygons for zonal stats."""
        gpkg_driver = ogr.GetDriverByName('GPKG')
        vector_path = os.path.join(self.workspace_dir, 'small_vector.gpkg')
        vector = gpkg_driver.CreateDataSource(vector_path)
    ...
    def test_zonal_stats_no_bb_overlap(self):
        """geoprocessing: test no vector bb raster overlap."""
        gpkg_driver = ogr.GetDriverByName('GPKG')
    ...
    def test_zonal_stats_all_outside(self):
        """geoprocessing: test vector all outside raster."""
        gpkg_driver = ogr.GetDriverByName('GPKG')
        vector_path = os.path.join(self.workspace_dir, 'vector.gpkg')
        vector = gpkg_driver.CreateDataSource(vector_path)
    ...
    def test_mask_raster(self):
        """geoprocessing: test mask raster."""
        origin_x = 1.0
        origin_y = -1.0
        n = 16
        test_val = 2
    ...
    def test_zonal_statistics(self):
        """geoprocessing: test zonal stats function."""
    ...
    def test_zonal_statistics_nodata(self):
        """geoprocessing: test zonal stats function with non-overlap."""
        # create aggregating polygon
        pixel_size = 30.0
        n_pixels = 9
        origin = (444720, 3751320)
    ...
    def test_zonal_statistics_nodata_is_zero(self):
        """geoprocessing: test zonal stats function w/ nodata set to 0."""
        # create aggregating polygon
        vector_path = os.path.join(self.workspace_dir, 'small_vector.gpkg')
        origin_x = 1.0
        origin_y = -1.0
    ...
    def test_zonal_statistics_named_layer(self):
        """geoprocessing: test zonal stats with named layer."""
        # create aggregating polygon
        pixel_size = 30.0
        n_pixels = 9
        origin = (444720, 3751320)
    ...
    def test_zonal_statistics_bad_vector(self):
        """geoprocessing: zonal stats raises exception on bad vectors."""
        # create aggregating polygon
        n_pixels = 9
        ...
        with self.assertRaises(RuntimeError) as cm:
            _ = geoprocessing.zonal_statistics(
    ...
    def test_zonal_statistics_bad_raster_path_band(self):
        """geoprocessing: test zonal stats with bad raster/path type."""
        pixel_size = 30.0
        n_pixels = 9
        origin = (444720, 3751320)
        polygon_a = shapely.geometry.Polygon([
    ...
    def test_interpolate_points(self):
        """geoprocessing: test interpolate points feature."""
        ...
        # interpolate
        geoprocessing.interpolate_points(
            source_vector_path, 'value', (result_path, 1), 'near')

        # verify that result is expected
        result_array = geoprocessing.raster_to_numpy_array(result_path)
    ...
    def test_invoke_timed_callback(self):
        """geoprocessing: cover a timed callback."""
        reference_time = time.time()
        time.sleep(0.1)
        new_time = geoprocessing.geoprocessing._invoke_timed_callback(
    ...
    def test_warp_raster_unusual_pixel_size(self):
        """geoprocessing: warp on unusual pixel types and sizes."""
    ...
    def test_warp_raster_0x0_size(self):
        """geoprocessing: test warp where so small it would be 0x0."""
        pixel_a_matrix = numpy.ones((5, 5), numpy.int16)
        target_nodata = -1
        base_a_path = os.path.join(self.workspace_dir, 'base_a.tif')
        _array_to_raster(
    ...
    def test_align_and_resize_raster_stack_bad_values(self):
        """geoprocessing: align/resize raster bad base values."""
        pixel_a_matrix = numpy.ones((5, 5), numpy.int16)
        target_nodata = -1
        base_a_path = os.path.join(self.workspace_dir, 'base_a.tif')
        _array_to_raster(
    ...
    def test_align_and_resize_raster_stack_duplicate_outputs(self):
        """geoprocessing: align/resize raster duplicate outputs."""
        pixel_a_matrix = numpy.ones((5, 5), numpy.int16)
        target_nodata = -1
        base_a_path = os.path.join(self.workspace_dir, 'base_a.tif')
        _array_to_raster(
    ...
    def test_align_and_resize_raster_stack_bad_mode(self):
        """geoprocessing: align/resize raster bad bounding box mode."""
        pixel_a_matrix = numpy.ones((5, 5), numpy.int16)
        target_nodata = -1
        base_a_path = os.path.join(self.workspace_dir, 'base_a.tif')
        _array_to_raster(
    ...
    def test_align_and_resize_raster_stack_bad_index(self):
        """geoprocessing: align/resize raster test intersection."""
        pixel_a_matrix = numpy.ones((5, 5), numpy.int16)
        target_nodata = -1
        base_a_path = os.path.join(self.workspace_dir, 'base_a.tif')
        _array_to_raster(
    ...
    def test_align_and_resize_raster_stack_int(self):
        ...
        for raster_index in range(len(target_raster_path_list)):
            target_raster_info = geoprocessing.get_raster_info(
                target_raster_path_list[raster_index])
            target_array = geoprocessing.raster_to_numpy_array(
                target_raster_path_list[raster_index])
    ...
    def test_align_and_resize_raster_stack_int_with_vectors(self):
        """geoprocessing: align/resize raster test inters. w/ vectors."""
        pixel_a_matrix = numpy.ones((5, 5), numpy.int16)
        target_nodata = -1
        base_a_path = os.path.join(self.workspace_dir, 'base_a.tif')
        _array_to_raster(
    ...
    def test_align_and_resize_raster_stack_no_overlap(self):
        """geoprocessing: align/resize raster no intersection error."""
        pixel_a_matrix = numpy.ones((5, 5), numpy.int16)
        target_nodata = -1
        base_a_path = os.path.join(self.workspace_dir, 'base_a.tif')
        _array_to_raster(
    ...
    def test_align_and_resize_raster_stack_union(self):
        """geoprocessing: align/resize raster test union."""
        pixel_a_matrix = numpy.ones((5, 5), numpy.int16)
        target_nodata = -1
        base_a_path = os.path.join(self.workspace_dir, 'base_a.tif')
        _array_to_raster(
    ...
    def test_align_and_resize_raster_stack_bb(self):
        """geoprocessing: align/resize raster test bounding box."""
        pixel_a_matrix = numpy.ones((5, 5), numpy.int16)
        target_nodata = -1
        base_a_path = os.path.join(self.workspace_dir, 'base_a.tif')
        _array_to_raster(
    ...
    def test_raster_calculator_no_path(self):
        """geoprocessing: raster_calculator raise ex. on bad file path."""
        target_nodata = -1
    ...
    def test_raster_calculator_nodata(self):
        """geoprocessing: raster_calculator test with all nodata."""
    ...
    def test_rs_calculator_bad_overlap(self):
        """geoprocessing: rs_calculator expected error on bad overlap."""
    ...
    def test_raster_calculator_constant_args_error(self):
        """geoprocessing: handle empty input arrays."""
        target_path = os.path.join(self.workspace_dir, 'target.tif')
        with self.assertRaises(ValueError) as cm:
    ...
    def test_raster_calculator_invalid_numpy_array(self):
        """geoprocessing: handle invalid numpy array sizes."""
        target_path = os.path.join(self.workspace_dir, 'target.tif')
    ...
    def test_raster_calculator_invalid_band_numbers(self):
        """geoprocessing: ensure invalid band numbers fail."""
    ...
    def test_raster_calculator_unbroadcastable_array(self):
        """geoprocessing: incompatable array sizes raise error."""
        target_path = os.path.join(self.workspace_dir, 'target.tif')
        a_arg = 3
    ...
    def test_raster_calculator_array_raster_mismatch(self):
        """geoprocessing: bad array shape with raster raise error."""
        base_path = os.path.join(self.workspace_dir, 'base.tif')
        _array_to_raster(
    ...
    def test_raster_calculator_bad_raw_args(self):
        """geoprocessing: tuples that don't match (x, 'raw')."""
        target_path = os.path.join(self.workspace_dir, 'target.tif')
        base_path = os.path.join(self.workspace_dir, 'base.tif')
        _array_to_raster(
            numpy.ones((128, 128), dtype=numpy.int32), -1, base_path)
    ...
    def test_raster_calculator_constant_args(self):
        """geoprocessing: test constant arguments of raster calc."""
        target_path = os.path.join(self.workspace_dir, 'target.tif')
        a_arg = 3
        x_arg = numpy.array(range(2))
        y_arg = numpy.array(range(3)).reshape((3, 1))
    ...
    def test_combined_constant_args_raster(self):
        """geoprocessing: test raster calc with constant args."""
        base_path = os.path.join(self.workspace_dir, 'base.tif')
        nodata = 0
        ...
        result = geoprocessing.raster_to_numpy_array(target_path)
        expected_result = (
    ...
    def test_new_raster_from_base_unsigned_byte(self):
        """geoprocessing: test that signed byte rasters copy over."""
    ...
    def test_new_raster_from_base_nodata_not_set(self):
        """geoprocessing: test new raster with nodata not set."""
        driver = gdal.GetDriverByName('GTiff')
        base_path = os.path.join(self.workspace_dir, 'base.tif')
        ...
        raster_properties = geoprocessing.get_raster_info(target_path)
        self.assertEqual(raster_properties['nodata'], [None])

    def test_create_raster_from_vector_extents(self):
        """geoprocessing: test creation of raster from vector extents."""
        point_a = shapely.geometry.Point(
            _DEFAULT_ORIGIN[0], _DEFAULT_ORIGIN[1])
        n_pixels_x = 9
        n_pixels_y = 19
        ...
        raster_properties = geoprocessing.get_raster_info(
            target_raster_path)
    ...
    def test_create_raster_from_vector_extents_invalid_pixeltype(self):
        """geoprocessing: raster from vector with bad datatype."""
        point_a = shapely.geometry.Point(
            _DEFAULT_ORIGIN[0], _DEFAULT_ORIGIN[1])
        n_pixels_x = 9
        n_pixels_y = 19
    ...
    def test_create_raster_from_vector_extents_odd_pixel_shapes(self):
        """geoprocessing: create raster vector ext. w/ odd pixel size."""
        point_a = shapely.geometry.Point(
            _DEFAULT_ORIGIN[0], _DEFAULT_ORIGIN[1])
        pixel_x_size = -10
        pixel_y_size = 20
    ...
    def test_create_raster_from_vector_extents_linestring_no_width(self):
        """geoprocessing: create raster from v. ext with no geom width."""
    ...
    def test_create_raster_from_vector_extents_linestring_no_height(self):
        """geoprocessing: create raster from v. ext with no geom height."""
        point_a = shapely.geometry.LineString(
            [(_DEFAULT_ORIGIN[0], _DEFAULT_ORIGIN[1]),
    ...
    def test_create_raster_from_vector_extents_bad_geometry(self):
        """geoprocessing: create raster from v. ext. with bad geometry."""
        ...
        result = geoprocessing.raster_to_numpy_array(target_raster_path)
        numpy.testing.assert_array_equal(expected_result, result)

    def test_transform_box(self):
        """geoprocessing: test geotransforming lat/lng box to UTM10N."""
        ...
        gibraltar_bb_wgs84 = geoprocessing.transform_bounding_box(
    ...
    def test_transform_box_latlon_to_utm(self):
        """geoprocessing: test geotransforming lat/lon box to UTM19N."""
    ...
    def test_transform_box_utm_to_latlon(self):
        """geoprocessing: test geotransforming UTM19N box to lat/lon."""
    ...
    def test_transform_box_latlon_to_latlon(self):
        """geoprocessing: test geotransforming lat/lon box to lat/lon."""
        # lat/lon bounds for Coast of New England from shapefile created in
        # QGIS 3.10.2 running GDAL 3.0.3
        bounding_box = [-72.3638439, 40.447948, -68.0041948, 43.1441579]
    ...
    def test_transform_box_utm_to_utm(self):
        """geoprocessing: test geotransforming utm box to utm."""
        # UTM19N bounds for Coast of New England
        bounding_box = [
            214722.122449, 4477484.382162, 584444.275934, 4782318.029707]
    ...
    def test_transform_bounding_box_out_of_bounds(self):
        """geoprocessing: test that bad transform raises an exception."""
    ...
    def test_iterblocks_unsigned_byte(self):
        """geoprocessing: test iterblocks with unsigned byte."""
        n_pixels = 100
    ...
    def test_convolve_2d_single_thread(self):
        """geoprocessing: test convolve 2d (single thread)."""
        n_pixels = 100
    ...
    def test_convolve_2d_normalize_ignore_nodata(self):
        """geoprocessing: test convolve 2d w/ normalize and ignore."""
        n_pixels = 100
        signal_array = numpy.ones((n_pixels, n_pixels), numpy.float32)
        test_value = 0.5
        signal_array[:] = test_value
    ...
    def test_convolve_2d_normalize(self):
        """geoprocessing: test convolve 2d w/ normalize."""
        n_pixels = 3
        signal_array = numpy.ones((n_pixels, n_pixels), numpy.float32)
        test_value = 0.5
        signal_array[:] = test_value
    ...
    def test_convolve_2d_missing_nodata(self):
        """geoprocessing: test convolve2d if target type but no nodata."""
        n_pixels = 100
        signal_array = numpy.ones((n_pixels, n_pixels), numpy.float32)
        test_value = 0.5
        signal_array[:] = test_value
    ...
    def test_convolve_2d_reverse(self):
        """geoprocessing: test convolve 2d reversed."""
        n_pixels = 3
        signal_array = numpy.ones((n_pixels, n_pixels), numpy.float32)
        test_value = 0.5
        signal_array[:] = test_value
    ...
    def test_convolve_2d_large(self):
        """geoprocessing: test convolve 2d with large kernel & signal."""
        n_pixels = 100
    ...
    def test_convolve_2d_numerical_zero(self):
        """geoprocessing: test convolve 2d for numerical 0.0 set to 0.0."""
    ...
    def test_convolve_2d_ignore_undefined_nodata(self):
        """geoprocessing: test convolve 2d ignore nodata when None."""
        # set tiny signal with one pixel on so we get lots of numerical noise
        n_pixels = 100
        n_kernel_pixels = 100
        signal_array = numpy.zeros((n_pixels, n_pixels), numpy.float32)
    ...
    def test_calculate_slope(self):
        """geoprocessing: test calculate slope."""
        n_pixels = 9
        ...
        geoprocessing.calculate_slope((dem_path, 1), target_slope_path)
        ...
        for _, band_data in geoprocessing.iterblocks(
                (target_slope_path, 1)):
    ...
    def test_calculate_slope_undefined_nodata(self):
        """geoprocessing: test calculate slope with no nodata."""
        n_pixels = 9
        dem_array = numpy.ones((n_pixels, n_pixels), numpy.float32)
        dem_path = os.path.join(self.workspace_dir, 'dem.tif')
        target_slope_path = os.path.join(self.workspace_dir, 'slope.tif')
        ...
        actual_slope = geoprocessing.raster_to_numpy_array(target_slope_path)
    ...
    def test_rasterize_error(self):
        """geoprocessing: test rasterize when error encountered."""
        n_pixels = 3
        target_raster_array = numpy.ones((n_pixels, n_pixels), numpy.float32)
        test_value = 0.5
        target_raster_array[:] = test_value
        ...
        with self.assertRaises(RuntimeError) as cm:
            ...
            with unittest.mock.patch(
                    'ecoshard.geoprocessing.geoprocessing._make_logger_callback',
    ...
    def test_rasterize_missing_file(self):
        """geoprocessing: test rasterize with no target raster."""
        n_pixels = 3
        target_raster_array = numpy.ones((n_pixels, n_pixels), numpy.float32)
        test_value = 0.5
        target_raster_array[:] = test_value
    ...
    def test_rasterize_error_handling(self):
        """geoprocessing: test rasterize error handling."""
        n_pixels = 3
        target_raster_array = numpy.ones((n_pixels, n_pixels), numpy.float32)
        test_value = 0.5
        target_raster_array[:] = test_value
    ...
    def test_distance_transform_edt(self):
        """geoprocessing: test distance transform EDT."""
        n_pixels = 1000
        target_nodata = 0
    ...
    def test_distance_transform_edt_small_sample_distance(self):
        """geoprocessing: test distance transform w/ small sample dist."""
        n_pixels = 10
        target_nodata = None
        base_raster_array = numpy.zeros(
            (n_pixels, n_pixels), dtype=numpy.int32)
    ...
    def test_distance_transform_edt_bad_data(self):
        """geoprocessing: test distance transform EDT with bad values."""
        n_pixels = 10
        base_raster_array = numpy.zeros(
            (n_pixels, n_pixels), dtype=numpy.int32)
        base_raster_array[int(n_pixels/2), int(n_pixels/2)] = 1
    ...
    def test_stitch_rasters_area_change(self):
        ...
        target_stitch_array = geoprocessing.raster_to_numpy_array(
            target_stitch_path)
    ...
    def test_stitch_rasters(self):
        """geoprocessing: test stitch_rasters."""
        wgs84_ref = osr.SpatialReference()
        wgs84_ref.ImportFromEPSG(4326)  # WGS84 EPSG
        ...
        geoprocessing.stitch_rasters(
            ...
            overlap_algorithm='etch')
        target_etch_array = geoprocessing.raster_to_numpy_array(
        ...
        target_replace_array = geoprocessing.raster_to_numpy_array(
            stitch_by_replace_target_path)
        ...
        target_add_array = geoprocessing.raster_to_numpy_array(
            stitch_by_add_target_path)
        # we add on the add
    ...
    def test_align_with_target_sr(self):
        ...
        target_raster_info = geoprocessing.get_raster_info(target_path)
    ...
    def test_get_raster_info_error_handling(self):
        ...
        with self.assertRaises(ValueError) as cm:
            geoprocessing.get_raster_info(not_a_raster_path)
        expected_message = 'Could not open'
        actual_message = str(cm.exception)
        self.assertTrue(expected_message in actual_message, actual_message)
    ...
    def test_get_vector_info_error_handling(self):
        ...
        with self.assertRaises(ValueError) as cm:
            geoprocessing.get_raster_info(not_a_vector_path)
        expected_message = 'Could not open'
        actual_message = str(cm.exception)
        self.assertTrue(expected_message in actual_message, actual_message)
    ...
    def test_align_and_resize_raster_stack_int_with_vector_mask_bb(self):
        ...
        poly_bb = [0, 0, 2, 2]
        poly_bb_transform = geoprocessing.transform_bounding_box(
    ...
    def test_align_and_resize_raster_stack_int_with_bad_vector_mask(self):
        """geoprocessing: align/resize raster w/ bad vector mask."""
        pixel_a_matrix = numpy.ones((5, 5), numpy.int16)
        target_nodata = -1
        base_a_path = os.path.join(self.workspace_dir, 'base_a.tif')
        pixel_size = 30
    ...
    def test_disjoint_polygon_set_no_bounding_box(self):
        """geoprocessing: check disjoint sets."""
        ...
        disjoint_sets = geoprocessing.calculate_disjoint_polygon_set(
            outflow_vector)
        self.assertEqual(
            disjoint_sets,
            [set([1, 2, 3, 5]), set([4])])
    ...
    def test_disjoint_polygon_set_no_features_error(self):
        """geoprocessing: raise an error when a vector has no features."""
        srs = osr.SpatialReference()
        ...
        with self.assertRaises(RuntimeError) as cm:
            geoprocessing.calculate_disjoint_polygon_set(empty_vector_path)
    ...
    def test_assert_is_valid_pixel_size(self):
        ...
        self.assertTrue(_assert_is_valid_pixel_size(
            (-10.5, 18282828228)))
        with self.assertRaises(ValueError) as cm:
    ...
    def test_percentile_int_type(self):
        ...
        actual_int_percentiles = geoprocessing.raster_band_percentile(
    ...
    def test_percentile_double_type(self):
        ...
        actual_percentiles = geoprocessing.raster_band_percentile(
    ...
    def test_percentile_unsupported_type(self):
        ...
        with self.assertRaises(ValueError) as cm:
            _ = geoprocessing.raster_band_percentile(
    ...
    def test_evaluate_raster_calculator_expression(self):
        ...
        target_nodata = None
        target_raster_path = os.path.join(self.workspace_dir, 'target.tif')
        ecoshard.geoprocessing.symbolic.evaluate_raster_calculator_expression(
        ...
        self.assertEqual(geoprocessing.get_raster_info(
    ...
    def test_get_gis_type(self):
        ...
        self.assertEqual(
            geoprocessing.get_gis_type(text_file_path),
            geoprocessing.UNKNOWN_TYPE)
        self.assertEqual(
            geoprocessing.get_gis_type(raster_path),
            geoprocessing.RASTER_TYPE)
        self.assertEqual(
            geoprocessing.get_gis_type(vector_path),
            geoprocessing.VECTOR_TYPE)

        with self.assertRaises(ValueError) as cm:
            geoprocessing.get_gis_type('totally_fake_file')
        actual_message = str(cm.exception)
    ...
    def test_iterblocks_bad_raster(self):
        ...
        with self.assertRaises(ValueError) as cm:
            _ = list(geoprocessing.iterblocks(('fake_file.tif', 1)))
    ...
    def test_warp_raster_signedbyte(self):
        ...
        base_array = geoprocessing.raster_to_numpy_array(base_a_path)
        numpy.testing.assert_array_equal(pixel_a_matrix, base_array)

        array = geoprocessing.raster_to_numpy_array(target_raster_path)
        numpy.testing.assert_array_equal(pixel_a_matrix, array)

    def test_convolve_2d_bad_input_data(self):
        """geoprocessing: test convolve 2d for programmer error."""
        signal_path = os.path.join(self.workspace_dir, 'signal.tif')
        kernel_path = os.path.join(self.workspace_dir, 'kernel.tif')
        target_path = os.path.join(self.workspace_dir, 'target.tif')
    ...
    def test_convolve_2d_interpolate_nodata(self):
        """geoprocessing: test ability to fill nodata holes."""
        signal_path = os.path.join(self.workspace_dir, 'signal.tif')
        kernel_path = os.path.join(self.workspace_dir, 'kernel.tif')
        target_path = os.path.join(self.workspace_dir, 'target.tif')
    ...
    def test_convolve_2d_gaussian(self):
        """geoprocessing: test convolve 2d with large gaussian kernel."""
    ...
    def test_convolve_with_byte_kernel(self):
        ...
        # numerical noise
        numpy.testing.assert_almost_equal(
            geoprocessing.raster_to_numpy_array(int_out_path),
            array)
