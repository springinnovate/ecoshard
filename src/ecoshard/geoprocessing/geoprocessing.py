...
def greedy_pixel_pick_by_area(
    ...
    Return:
        ``None```
    """
    base_raster_info = get_raster_info(base_value_raster_path_band[0])
    base_origin = (
        base_raster_info['geotransform'][0],
        base_raster_info['geotransform'][3])
    base_raster = gdal.OpenEx(base_value_raster_path_band[0])
    base_band = base_raster.GetRasterBand(base_value_raster_path_band[1])
    area_per_pixel_raster = gdal.OpenEx(area_per_pixel_raster_path_band[0])
    area_per_pixel_band = area_per_pixel_raster.GetRasterBand(
        area_per_pixel_raster_path_band[1])

    array_shape = (base_band.YSize, base_band.XSize)
    n_pixels = numpy.prod(array_shape)
    base_array = base_band.ReadAsArray().astype(numpy.float64)
    area_per_pixel_array = area_per_pixel_band.ReadAsArray().astype(
        numpy.float64)
    mask_array = numpy.zeros(array_shape, dtype=numpy.int8)

    # get sorted order base array but ignore nodata
    i_coords, j_coords = numpy.meshgrid(
        numpy.arange(base_band.XSize),
        numpy.arange(base_band.YSize))

    base_nodata = base_band.GetNoDataValue()
    if base_nodata is not None:
        nodata_mask = numpy.isclose(base_array, base_nodata)
    else:
        nodata_mask = numpy.ones(array_shape, dtype=bool)
    finite_mask = numpy.isfinite(base_array)
    valid_mask = finite_mask & ~nodata_mask

    # sort in decreasing order
    sort_indexes = numpy.argsort(-1*base_array[valid_mask])
    sorted_area = area_per_pixel_array[valid_mask][sort_indexes]

    sorted_area_sum = numpy.cumsum(sorted_area)
    ...
    for area_threshold_index in area_threshold_index_array:
        if area_threshold_index >= n_pixels:
            area_threshold_index = n_pixels-1
        ...
        numpy_array_to_raster(
            mask_array, None, base_raster_info['pixel_size'],
            base_origin, base_raster_info['projection_wkt'],
            mask_raster_path)

        with open(table_path, 'a') as csv_file:
            csv_file.write(
                f'{current_area},{value_sum[area_threshold_index]}\n')
